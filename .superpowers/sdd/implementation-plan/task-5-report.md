...
`pnpm --filter @digital-twin/desktop tauri build --bundles app` rebuilt `target/release/bundle/macos/Digital Twin Workbench.app` successfully after the capability change. The focused ACL suite is 2/2 green. This follow-up does not claim a real account login or open an external page: doing so would create a user session and is outside a non-interactive smoke. The compiled IPC smoke and fresh `.app` build establish the authorization boundary; a user can complete the normal login flow with an official Codex-returned URL. Re-review updated the desktop configuration regression from a stale two-string count to the same minimal declarative boundary; desktop is now 68/68 while it still rejects opener default/HTTP/path/reveal/shell/filesystem expansion. The latest pre-publish source and bundle Workers both hash to `2633274138eaeba33fc370b7adbf680299c07ae27bdb874402c35efba1750f23`; SEA rebuilding/ad-hoc signing can change a per-build hash, so only same-build byte equality is the acceptance contract.
