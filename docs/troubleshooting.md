...
大纲批准后进入逐页细化，并可展开“已批准大纲（只读）”回看。已有大纲无需重新上传或生成；重新打开新版应用即可使用新界面。本次仅更新大纲审核，材料分析和逐页细化的 JSON 展示尚未替换。

有未保存的大纲时，需要先保存它，才能保存可能清除大纲的生成说明。返回项目列表、侧栏跳转及浏览器返回会提醒未保存修改；关闭 macOS 原生窗口前仍请主动保存，浏览器离开保护不等同于原生窗口关闭保护。
