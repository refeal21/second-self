...
Final review Ruling: the sole residual is real and load-bearing — the Tauri UI calls `opener.openUrl` for ChatGPT login, but the main-window capability does not grant the opener command/safe HTTPS scope, so a logged-out user cannot complete the product's required login flow. The branch must not be pushed as finished until a new user-authorized follow-up adds the narrowly scoped opener permission and a packaged ACL smoke test; the final-review workflow permits no second unreviewed fix wave. All original five Critical and four Important findings are independently verified addressed.
...
Task 5 login ACL follow-up RED → GREEN (2026-09-04, no commit): Tauri's effective ACL resolver first proved the main window had no `plugin:opener|open_url`, matching the production login failure. The minimal replacement grants only `opener:allow-open-url` to the system default browser for `https://auth.openai.com/*`, `https://chatgpt.com/*`, and `https://chat.openai.com/*`; it grants no HTTP, file/path, reveal, shell, filesystem, or arbitrary-app access. The focused parser/compiled-IPC suite is 2/2 and rejects `http:`, `file:`, unrelated and deceptive domains. A fresh macOS `.app` build completed. The IPC smoke deliberately invokes only a forbidden `file:` URL so it proves command authorization reaches opener scope without opening a browser or creating a login session; no live-login claim is made. Re-review replaced the stale event-only desktop assertion with a minimal declarative-boundary check; desktop full suite is 68/68 without changing production permissions.

Task 5 latest pre-publish bundle evidence (2026-09-04): the current source SEA and matching `.app` bundle Worker SHA-256 are both `2633274138eaeba33fc370b7adbf680299c07ae27bdb874402c35efba1750f23`. SEA rebuilds and ad-hoc signatures can change a build's exact hash; acceptance requires source/bundle byte equality within the same build. Earlier `cc2306620b3708c8d19b0c909769bbc7d2033c7dea938e5c6fe94e3b92db45cb` evidence remains historical only.
