...
大纲和细化均使用中文逐页表单，不必编辑 JSON。细化支持增删、排序及正文/表格/图表/形状修改；内容保存不调用 AI。结构变更须先确认新大纲，再单独批准细化，旧版本和审批保留。操作方法见[逐页细化审核](docs/troubleshooting.md#如何审核逐页细化)。

### 整页图片与模板配色

在 PPT 项目展开“项目配色与模板参考”，可以选择 `.pptx` 作为**配色参考**。模板只在本地解析，不作为事实材料；候选颜色不会自动应用。请确认主色、背景色、文字色和辅助色后显式保存，再生成或重新生成当前页。主色用于标题与强调，不是给整页铺同一种颜色。未选择模板时也可手动指定配色。

生成图片现在要求包含已批准的标题、正文、数据与版式，不是仅生成无字背景。已批准的内容不会因配色变化而改写；历史图片保留，需手动重新生成。第一张图片批准后，本项目配色锁定；修改前已生成的旧配色候选不能直接批准。

新候选旁可查看实际发出的应用提示词、修改意见、配色版本和生成来源；旧图片没有记录的依据会明确标为未知，不倒推或补造。检查文字/数据完整和排版/配色后，勾选两项人工检查才能批准。这些检查不代表 OCR 自动逐字验收。

本阶段只实现模板配色参考；**把图片插入原始模板版式、保留模板页眉/标识，以及整页 PNG 转换后的可编辑版式一致性仍待下一阶段验收**。备份时须同时保存应用 SQLite 和项目工作区；仅备份旧的 pipeline JSON 不包含独立保存的配色与生成记录。
